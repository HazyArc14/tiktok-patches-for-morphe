...
| `Share sheet modification` | Toggles the share sheet's "Send to", "Share via app", "Video Actions", "User Actions", and "Live Actions" sections, with an allow-list for which apps and actions appear in each. Optionally adds a Block action to a video's share sheet. |
